...
try:
    ...
    logger.warning("SALib library not found. Sobol visualization will not be available.")
...
def _create_cache_key(
    ...
    data_hash = hashlib.md5(np.ascontiguousarray(data_array[:100]).tobytes()).hexdigest()  # 只使用前100个样本

    key_parts = [model_id, data_hash, str(model_type), str(n_samples), str(output_dimension)]
    return "_".join(key_parts)


def _get_model_predict_function(model: Any, model_type: Optional[str], output_dimension: Optional[int] = None):
    """
    获取模型的预测函数
    ...
    if model_type is None:
        ...
        if 'sklearn' in str(model.__class__.__module__).lower() or any(x in model_class for x in ['forest', 'tree', 'boost', 'linear']):
            model_type = 'sklearn'
...
def _calculate_sobol_indices(
    ...
    if not SALIB_AVAILABLE:
        raise ImportError("SALib library is required. Install with 'pip install SALib'.")
...
def _get_sobol_indices(
    ...
    if not use_cache:
        return _calculate_sobol_indices(
            input_data, model, model_type, n_samples,
            output_dimension, feature_names, calc_second_order
        )

    # 创建缓存键
    cache_key = _create_cache_key(model, input_data, model_type, n_samples, output_dimension)
...
def get_sobol_indices(
    ...
    sobol_indices = _get_sobol_indices(
        input, model, model_type, n_samples,
        output_dimension, feature_names, calc_second_order, use_cache
    )
...
def plot_sobol_indices(
    ...
    sobol_indices = _get_sobol_indices(
        input, model, model_type, n_samples,
        output_dimension, feature_names, True, use_cache
    )
    ...
    # 排序特征
    if sort_features:
        importance_df = _rank_features_by_importance(indices, names, conf, normalized=False)
        sorted_names = importance_df['Feature'].tolist()
        sorted_indices = importance_df['Importance'].to_numpy()
        sorted_conf = importance_df['Confidence'].to_numpy() if 'Confidence' in importance_df.columns else None
    else:
        sorted_names = names
...
def plot_sobol_interactions(
    ...
    if 'S2' not in sobol_indices:
        raise ValueError("Second-order indices not available. Set calc_second_order=True.")
